...
class NewsCrawler(ABC):
    ...
    def get_all_articles_of_month(self, year, month):
        start_date = datetime(year, month, 1).date()
        if month == 12:
            end_date = datetime(year + 1, 1, 1).date() - timedelta(days=1)
        else:
            end_date = datetime(year, month + 1, 1).date() - timedelta(days=1)
        return self.get_all_articles_between_dates(start_date, end_date)
    
    def get_all_articles_between_dates(self, start_date, end_date):
        articles = []
        current_date = start_date
        while current_date <= end_date:
            articles.extend(self.get_all_articles_of_date(current_date))
